...
Therapy Wiki 用尽量轻量的工作流来解决问题：一棵文件树加一组 Codex / Claude 的 skills 就完事了。没有 hosted service、web UI、向量数据库、要维护的后端（这些东西在最近几个月开启的 AI agent 时代已经过时了！😱）如果你已经在用 Codex 或 Claude Code，交互界面就是现成的。
...
把音频丢进仓库，流水线会自动转写、做说话人分离、生成 review 产物，最后落到一份长期维护的 Markdown wiki 里，并跟已有的主题 / 模式页互相 cross-link。手动修正（transcript 编辑、speaker 标签）是一等公民；`refresh` 步骤可以只重编某一次 session。整套操作分五步 —— **Ingest → Review → Rebuild → Report → Discuss** —— 都由一个 `./therapy` CLI 和一组 Codex / Claude skills 驱动。

### 对同一材料的多重视角（Personas）

每个 persona 都有自己的方法卡和 skill，结构、问题、关注点、输出框架都不一样，不是同一套模型换个 prompt。
...
需要新的 persona？让 agent 直接加就行。

### 报告和讨论

- **单次复盘** —— 针对一次 session 的结构化输出。
- **纵向报告** —— 对 wiki 里所有 session 做综合。
- **Persona 讨论** —— 让某一个视角回答具体问题，必要时用 `--file-back` 把值得保留的结论写回该 persona 的 wiki 笔记。

报告默认中文输出（要换语言跟 agent 说一声就行），source-grounded，每个判断都要引用 `session_id + timestamp`。
...
新机器第一次配 STT + diarization（`mlx-whisper`、`pyannote` 等）见 [schema/setup.md](./schema/setup.md)；CLI 完整行为见 [schema/cli.md](./schema/cli.md)；wiki 维护规则见 [schema/wiki-maintainer.md](./schema/wiki-maintainer.md)。

</details>

### 日常使用

日常使用不需要记任何命令行参数。在仓库目录里打开 Codex 或 Claude Code，直接用自然语言告诉 agent 三件事：**scope（针对哪些材料）** × **persona（用什么视角）** × **task（做什么）**。agent 会自己选 skill、调用 CLI。
...
**可选的 persona** —— 与上方表格一致：`therapist` / `supervisor` / `psychologist` / `intp-lens` / `close-friend`。
...
<details>
<summary>对应的 <code>./therapy</code> 命令（给 agent 看）</summary>
...
`ingest` 步骤是一条确定性的本地流水线，核心完全跑在开源模型上 —— 没有云端 STT、没有云端 LLM、零 API 费用。
